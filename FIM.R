funFIMsm <- function(equation,paramName,parameter,sigma,t_indiv,d,nbSubjects){
  ...
  if(sig.slope ==0){
    M_F <- M_F[-c(lpsi),-c(lpsi)]
    PSI <- PSI[-c(lpsi)]
  }
  if(sig.inter == 0){
    M_F <- M_F[-c(lpsi-1),-c(lpsi-1)]
    PSI <- PSI[-c(lpsi-1)]
  }
...
#exercice1
#funFIMsm("dose/V*ka/(ka-(Cl/V))*(exp(-(Cl/V)*t)-exp(-ka*t))",c("ka","V","Cl"),c(1.6,8,0.13),c(0.6,0.07),list(c(0.5, 1, 2, 6, 9, 12, 24, 36, 48, 72, 96, 120)),c(100),c(32))
#funFIMsm("dose/V*ka/(ka-(Cl/V))*(exp(-(Cl/V)*t)-exp(-ka*t))",c("ka","V","Cl"),c(1.6,8,0.13),c(0.6,0.07),list(c(1)),c(100),c(1))
