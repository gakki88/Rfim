...
# if(sig.slope ==0){
#   PSI<-c(paramName,"sig.inter")
#   form2 <- paste0("( sig.inter)^2")
# }
# if(sig.inter == 0){
#   PSI<-c(paramName,"sig.slope")
#   form2 <- paste0("(sig.slope * fixed )^2")
# }
...
mdfi <- mdf[,-c(length(paramName)+1,length(paramName)+2)]
...
# sink('D:\\yuxin\\code\\testFim\\rfimpdose.txt')
