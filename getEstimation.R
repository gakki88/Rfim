...
for(i in 1:4){
  ree<-c(ree,(gg[5+i,]-omega[i])/omega[i]*50)
  ree2<-cbind(ree2,(gg[5+i,]-omega[i])/omega[i]*50)
}
...
ggplot(ree1) +aes(x=parameters, y=values,color="") +
  ...
  scale_x_discrete( labels=c(expression(mu["VA"[0]]),expression(mu["k"]),expression(mu[beta]),expression(mu["E"["max"]]),expression(mu["ED"[50]]),expression(omega["VA"]),expression(omega["k"]),expression(omega[beta]),expression(omega["E"["max"]]),expression(sigma["i"])))
...
barplot(RSE2,beside = T,col=c("darkblue","red"),legend=T,ylim=c(0,60),names=c(expression(mu["VA"[0]]),expression(mu["k"]),expression(mu[beta]),expression(mu["E"["max"]]),expression(mu["ED"[50]]),expression(omega["VA"]^{2}),expression(omega["k"]^{2}),expression(omega[beta]^{2}),expression(omega["E"["max"]]^{2}),expression(sigma["i"])))
